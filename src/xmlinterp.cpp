 ...
 * dodatkowe pola.
 */
XMLInterp4Config::XMLInterp4Config(Configuration *pconfig)
{
	pConfig = pconfig;
}
...
void XMLInterp4Config::ProcessLibAttrs(const xercesc::Attributes  &Attrs)
{
 ...
 pConfig->AddLib(string(sLibName));
//
//cout << "ser" << endl;
...
*/
 ...
 // Tu trzeba wstawić odpowiednio własny kod ...
	...
	IStrm >> _Pozycja[0] >> _Pozycja[1] >> _Pozycja[2];
	if (IStrm.fail()) 
	{
     cerr << " Blad!!!" << endl;
	}
	...
	{
     cout << " Czytanie wartosci OK!!!" << endl;
     cout << _Pozycja[0] << ' ' << _Pozycja[1] << ' ' << _Pozycja[2]<< endl;
	}
	
	IStrm >> _RGB[0] >> _RGB[1] >> _RGB[2];
	
	pConfig->CreateObject(string(sValue_Name),_Pozycja, _RGB);
                                       ...
                                       )
{
  ...
  if (ElemName == "Lib") {
    ...
    ProcessLibAttrs(Attrs);  //cout << "ser" << endl; return;   
  }
...
{
  cerr << "Ostrzezenie z paser xml" << endl;
